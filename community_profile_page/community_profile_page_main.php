...
$stmtPostCount = $conn->prepare("SELECT COUNT(*) AS postCount FROM posts WHERE email = ?");
$stmtPostCount->bind_param("s", $userEmail);
...
$sqlUser = "SELECT username, lastname, image FROM users WHERE email = ?";
...
if ($resultUser->num_rows > 0) {
    ...
    $firstName = htmlspecialchars($user['username']);
    $lastName = htmlspecialchars($user['lastname']);
    $profilePic = '../profileimg/' . htmlspecialchars($user['image']); // Adjust the path as necessary
} else {
    // Handle case where user is not found
    $firstName = 'Firstname';
    $lastName = 'Last Name';
    $profilePic = '../img/noprofile.jpg'; // Default profile picture
}
...
<div class="middle1">
    <div class="story-section">
        <div class="card1">
            ...
            <div class="content">
                <div class="details">
                    <h2><?php echo $firstName; ?> <br><span><?php echo $lastName; ?></span></h2>
                    <div class="data">
...
// Fetch data from the 'posts' table where the email matches the logged-in user's email, ordered by time in descending order
$sql = "SELECT * FROM posts WHERE email = ? ORDER BY time DESC";
