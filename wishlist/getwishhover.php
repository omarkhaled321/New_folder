...
if (!isset($_SESSION['email'])) {
    ...
    // User is logged in, proceed with displaying wishlist content
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Output store name on top of the image
    echo "<div class='store-name'>" . $row["storename"] . "</div>"; // Assuming 'storename' is the column name in the database
    ...
    // Calculate subtotal
    $subTotal += $row['price']; // Assuming 'price' is the column name in the database
}
        ...
        echo "<a href='#' class='primary-button add-all-to-cart'>Add All Items To Cart</a>";
        echo "<a href='../cart/cart.php' class='secondary-button'>View Cart</a>";
        echo "</div>";
        echo "</div>";
    } else {
...
function sendToServer(wishlistId, productId, action) {
    ...
    xhr.open('GET', '../wishlist/addwish.php?wishlistId=' + wishlistId + '&productId=' + productId + '&action=' + action, true);
